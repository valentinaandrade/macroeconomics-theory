...
kk(i) = sum(mt.*((alpha)./(r(i)+delta)).^(1/(1-alpha)).*gammat(i,:));% sacarle el w de antes y agregrar ((1- alpha)*((alpha)/(r+delta)).^(1/(1-alpha))).*
end
toc
...
[~, ~, ~, ~, lt_activos, lt_consumo(i,:), ~,~, omega(i,:)] = fisher(T, sigma, beta,r_eq(i),liq(i));
end
