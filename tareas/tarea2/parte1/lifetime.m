function [vt,Ac, Ap, Cp, lt_activos, lt_ahorro, lt_consumo] = lifetime(A,T, sigma, beta,r,w,liq)
...
for t = T:-1:1 % T entre T maximo hasta 1 con un espaciado de -1
    h(t) = (h(t+1)-w(t))/(1+r); %  Deuda maxima en cada periodo. Entonces si 
...
%c_70 = w(70) + (1+r).*agrid; % en el ultimo periodo me lo como todo
%c_70(c_70<0)=NaN;
...
for t = T:-1:1   
        ...
        c = w(t) + (1+r).*A(h_pos:end) - A(h_pos_p:end)'; % consumo fatible no es óptimo
        ...
        Cp(h_pos:end,t) = w(t) + (1+r).*A(h_pos:end) - A(h_pos_p - 1 + pos); % Cuando se despeja consumo
...
lt_consumo = w(1:T) + (1+r).*lt_activos(1:T) - lt_activos(2:T+1); %Policy function consumption

% Ahorro - lt_ahorro -----------------------------------------------------
% Lo mismo para ahorro
lt_ahorro = w(1:T) - lt_consumo; %Policy function ahorro
end