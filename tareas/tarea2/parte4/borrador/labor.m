...
c_final=(1+r)*A'+(y(T)*(1-l_final)); % se evalua el c_final y l_final
...
tic
for t=T-1:-1:1      
    c=(1/(1+varphi))*((1+r)*A'+y(t)-A); % restriccion + CPO2
    ...
    c=(1-l)*y(t)+(1+r)*A'-A; % se evalúa el l optimo para obtener c
    c(c<0)=NaN;
